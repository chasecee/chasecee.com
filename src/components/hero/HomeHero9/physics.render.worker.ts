...
let dt = 1 / 60;
let accumulator = 0;
let simStepCounter = 0;
let worldStepCounter = 0;
...
function applyIntegrationParams(settings: PhysicsSettings) {
  if (!world) return;
  const ip = world.integrationParameters as any;
  const sim = settings.simulation as any;
  ip.allowedLinearError = 0.05;
  ip.maxPenetrationCorrection = 0.05;
  ip.maxPositionIterations = sim.maxPositionIterations ?? 8;
...
function setupWebgl() {
  ...
  if (shapeSidesUniform) {
    gl.uniform1i(shapeSidesUniform, 0);
  }
...
function getPlanetRadiusPixels(settings: PhysicsSettings) {
...
function update(currentTime: number) {
  ...
  if (accumulator >= dt) {
    accumulator -= dt;
    const simTick = simStepCounter;
    const worldStepFrameSkip = Math.max(
      1,
      (settings.simulation as any).worldStepFrameSkip ?? 1,
    );
    const shouldRunWorldStep = simTick % worldStepFrameSkip === 0;
    simStepCounter++;
    didStep = shouldRunWorldStep;
    if (!shouldRunWorldStep) {
      const simulationTime = performance.now() - simStart;
      const totalTime = simulationTime;
      const fpsCalc = frameTime > 0 ? 1 / frameTime : 0;
      if (IS_DEV && frameStart - lastMetricsPost >= DEV_METRICS_INTERVAL_MS) {
        lastMetricsPost = frameStart;
        self.postMessage({
          type: "METRICS",
          simulationTime,
          renderTime: 0,
    ...
    const gravityEase = (settings.simulation as any).gravityEase ?? 1;
    const gravityFrameSkip = Math.max(
      1,
      (settings.simulation as any).gravityFrameSkip ?? 1,
    );
    const shouldApplyGravity = worldStepCounter % gravityFrameSkip === 0;
    const gravityStart = IS_DEV ? performance.now() : 0;
    ...
    const stepStart = IS_DEV ? performance.now() : 0;
    world.integrationParameters.dt = dt;
    world.step();
    if (IS_DEV) stepTime = performance.now() - stepStart;
  }
  const simEnd = performance.now();

  const maxSpeedCfg = (settings.simulation as any).maxSpeed;
  const maxSpeedClampFrameSkip = Math.max(
    1,
    (settings.simulation as any).maxSpeedClampFrameSkip ?? 1,
  );
  const shouldClampSpeed =
    didStep && worldStepCounter % maxSpeedClampFrameSkip === 0;
  const hasMaxSpeed =
    shouldClampSpeed && typeof maxSpeedCfg === "number" && maxSpeedCfg > 0;
  ...
  if (didStep) {
    for (let i = 0; i < bodyCount; i++) {
      const body = rigidBodies[i];
      if (!body) continue;
      if (hasMaxSpeed && !body.isSleeping()) {
        const v = body.linvel();
  ...
  if (didStep && gl && interleavedVbo) {
    const uploadStart = IS_DEV ? performance.now() : 0;
    uploadInterleavedData(bodyCount * DYNAMIC_BYTES);
    if (IS_DEV) uploadTime = performance.now() - uploadStart;
  }
...
function handleShockwave(
  ...
  const radiusMul = msg.radiusMul ?? 1;
  const forceMul = msg.forceMul ?? 1;
  const shockRadiusPixels =
    shockConfig.radius * Math.min(canvasWidth, canvasHeight) * radiusMul;
  ...
  for (const body of rigidBodies) {
    ...
    if (distSq < shockRadiusMeters * shockRadiusMeters) {
      ...
      const baseImpulse = shockConfig.force * falloff * strengthMul * forceMul;
...
async function handleInit(msg: Extract<MainToWorkerMessage, { type: "INIT" }>) {
  try {
    ...
    world.integrationParameters.dt = dt;
    applyIntegrationParams(settingsInit);
