...
const writeMorph = async (variants: MorphVariant[], cycleMs: number) => {
  ...
  await writeSplitModules(variants, cycleMs);
  return Buffer.byteLength(body);
};

/**
 * The runtime imports the morph data split in two: a small eager module with
 * ids + final paths (needed for first paint), and one lazy chunk per variant
 * with the heavy interpolation glyphs (only needed while morphing).
 */
const writeSplitModules = async (variants: MorphVariant[], cycleMs: number) => {
  const glyphsDir = path.join(VARIANTS_DIR, "glyphs");
  await mkdir(glyphsDir, { recursive: true });

  const meta =
    `// Generated by scripts/bake-logo-morph.ts — do not edit.\n` +
    `export const MORPH_POINT_COUNT = ${POINTS};\n` +
    `export const MORPH_CYCLE_MS = ${cycleMs};\n` +
    `export const MORPH_VARIANT_IDS = ${JSON.stringify(variants.map((v) => v.id))};\n` +
    `export const MORPH_PATHS = ${JSON.stringify(variants.map((v) => v.paths))};\n`;
  await writeFile(path.join(VARIANTS_DIR, "morphMeta.js"), meta);

  for (const variant of variants) {
    await writeFile(
      path.join(glyphsDir, `${variant.id}.js`),
      `// Generated by scripts/bake-logo-morph.ts — do not edit.\n` +
        `export const GLYPHS = ${JSON.stringify(variant.glyphs)};\n`,
    );
  }

  const loaders = variants
    .map(
      (variant) =>
        `  ${JSON.stringify(variant.id)}: () => import("./${variant.id}.js").then((m) => m.GLYPHS),`,
    )
    .join("\n");
  await writeFile(
    path.join(glyphsDir, "index.js"),
    `// Generated by scripts/bake-logo-morph.ts — do not edit.\n` +
